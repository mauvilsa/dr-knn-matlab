function [bestWW, bestU, bestV] = sfma2(POS, NEG, WW0, U0, V0, varargin)
%
% SFMA: Score Fusion by Maximizing the AUC
%
% [WW, U, V] = sfma(POS, NEG, WW0, U0, V0, ...)
%
%   Input:
%     POS                  - Positive scores matrix
%     NEG                  - Negative scores matrix
%     WW0                  - Initial score weights
...
%
%   Output:
%     WW                   - Final learned score weights
...
if islogical(WW0) && WW0==true,
  WW0=power(10,6.*(auc(POS,NEG)-0.5));
end
...
elseif size(WW0,1)~=1 || size(U0,1)~=1 || size(V0,1)~=1 || size(WW0,2)~=D || size(U0,2)~=D || size(V0,2)~=D,
  fprintf(logfile,'sfma: error: WW0, U0 and V0 must be row vectors and have the same dimensionality\n');
elseif size(POS,2)~=D || size(NEG,2)~=D,
  ...
  while 1,
    ...
    W0=2.*W.*overN.*(cPOS'*nPOS-cNEG'*nNEG);
    nPOS=power(nPOS,2).*(1./nPOS-1);
    nPOS(isnan(nPOS))=0;
    nNEG=power(nNEG,2).*(1./nNEG-1);
    nNEG(isnan(nNEG))=0;
    U0=WW.*overN.*(cPOS'*((POS-V(ones(NP,1),:)).*nPOS)-cNEG'*((NEG-V(ones(NN,1),:)).*nNEG));
    V0=WW.*U.*overN.*(cNEG'*nNEG-cPOS'*nPOS);
  ...
  bestWW=bestWW./sum(bestWW);

  if exist('sd'),
    if sum(sd==0)>0,
      WWUV=[bestWW;bestU;bestV];
      D=size(sd,2);
      bestWW=zeros(1,D);
      bestU=zeros(1,D);
      bestV=zeros(1,D);
