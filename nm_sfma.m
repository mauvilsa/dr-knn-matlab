function [bestW, bestU, bestV] = nm_sfma(POS, NEG, W0, U0, V0, varargin)
%
...
%     U0                   - Initial sigmoid slopes
%     V0                   - Initial sigmoid displacements
%
%   Input (optional):
...
elseif size(W0,1)~=1 || size(U0,1)~=1 || size(V0,1)~=1 || size(W0,2)~=D || size(U0,2)~=D || size(V0,2)~=D,
  fprintf(logfile,'sfma: error: W0, U0 and V0 must be row vectors and have the same dimensionality\n');
elseif size(POS,2)~=D || size(NEG,2)~=D,
  ...
  while 1,
    ...
    if devel,
      nePOS=1./(1+exp(U(ones(devNP,1),:).*(V(ones(devNP,1),:)-devPOS)));
      ePOS=nePOS*W';
      neNEG=1./(1+exp(U(ones(devNN,1),:).*(V(ones(devNN,1),:)-devNEG)));
      eNEG=neNEG*W';
    ...
    if approx,
      ...
      nPOS=1./(1+exp(U(ones(origNP,1),:).*(V(ones(origNP,1),:)-origPOS)));
      fPOS=nPOS*W';
      nNEG=1./(1+exp(U(ones(origNN,1),:).*(V(ones(origNN,1),:)-origNEG)));
      fNEG=nNEG*W';
      ...
      nPOS=1./(1+exp(U(ones(NP,1),:).*(V(ones(NP,1),:)-POS)));
      fPOS=nPOS*W';
      nNEG=1./(1+exp(U(ones(NN,1),:).*(V(ones(NN,1),:)-NEG)));
      fNEG=nNEG*W';
    ...
    for p=1:NP,
      expo=exp(beta*(fNEG-fPOS(p)));
      fact=beta.*expo./power(1+expo,2);
      cPOS(p)=cPOS(p)+sum(fact);
      cNEG=cNEG+fact;
      fact=beta.*fact.*(2.*expo./(1+expo)-beta);
    ...
    ccPOS=ccPOS(ones(D,1),:);
    ccNEG=ccNEG(ones(D,1),:);

    dW=overN.*(cPOS'*nPOS-cNEG'*nNEG);

    tmp=[ccPOS.*nPOS;ccNEG.*nNEG]'*[nPOS;nNEG];
    ddW=overN.*(nPOS'*(ccPOS.*nPOS)+nNEG'*(ccNEG.*nNEG)-tmp-tmp');

    dnPOS=power(nPOS,2).*(1./nPOS-1);
    dnPOS(isnan(dnPOS))=0;
    dnNEG=power(nNEG,2).*(1./nNEG-1);
    dnNEG(isnan(dnNEG))=0;

    POS0=POS-V(ones(NP,1),:);
    NEG0=NEG-V(ones(NN,1),:);

    cnPOS=(POS-V(ones(NP,1),:)).*dnPOS;
    cnNEG=(NEG-V(ones(NN,1),:)).*dnNEG;

    dU=overN.*W.*(cPOS'*cnPOS-cNEG'*cnNEG);
    dV=overN.*W.*U.*(cNEG'*dnNEG-cPOS'*dnPOS);

    %tmp=[ccPOS.*cnPOS;ccNEG.*cnNEG]'*[cnPOS;cnNEG]; %% wrong
    tmp=[ccPOS.*cnPOS;zeros(NN-NP,D)]'*cnNEG; %% right??
    ddU=(W'*W).*((ccPOS.*cnPOS)'*cnPOS+(ccNEG.*cnNEG)'*cnNEG-tmp-tmp');

    %tmp=[ccPOS.*dnPOS;ccNEG.*dnNEG]'*[dnPOS;dnNEG]; %% wrong
    tmp=[ccPOS.*dnPOS;zeros(NN-NP,D)]'*dnNEG; %% right??
    ddV=((W.*U)'*(W.*U)).*(dnPOS'*(ccPOS.*dnPOS)+dnNEG'*(ccNEG.*dnNEG)-tmp-tmp');

    dUdW=W(ones(D,1),:)'.*((ccPOS.*cnPOS)'*nPOS+(ccNEG.*cnNEG)'*nNEG-[ccPOS.*cnPOS;zeros(NN-NP,D)]'*nNEG-(ccNEG.*cnNEG)'*[nPOS;zeros(NN-NP,D)]);
    dUdW=overN*(dUdW+diag(cPOS'*cnPOS-nNEG'*cnNEG));

    %tmp=[ccPOS.*dnPOS;ccNEG.*dnNEG]'*[cnPOS;cnNEG]; %% wrong
    tmp=[ccPOS.*dnPOS;zeros(NN-NP,D)]'*cnNEG; %% right??
    dVdU=(W'*W).*U(ones(D,1),:)'.*((ccPOS.*dnPOS)'*(ccPOS.*cnPOS)+(ccNEG.*dnNEG)'*(ccNEG.*cnNEG)-tmp-tmp'); % i think it is wrong

    dVdW=repmat((W.*U)',1,D).*((ccPOS.*dnPOS)'*nPOS+(ccNEG.*dnNEG)'*nNEG-[ccPOS.*dnPOS;zeros(NN-NP,D)]'*nNEG-(ccNEG.*dnNEG)'*[nPOS;zeros(NN-NP,D)]);
    dVdW=overN*(dVdW+diag(U.*(cPOS'*dnPOS-cNEG'*dnNEG)));

    dnPOS=dnPOS*(1-2*nPOS*(1./nPOS-1));
    dnNEG=dnNEG*(1-2*nNEG*(1./nNEG-1));
    
    
    
    
    H=[ddW,dUdW',dVdW';dUdW,ddU,dVdU';dVdW,dVdU,ddV];
    P=inv(H)*[dW';dU';dV'];
    W=W+gamma.*P(1:D)';
    U=U+rho.*P(D+1:D+D)';
    V=V+sigma.*P(D+D+1:end)';
